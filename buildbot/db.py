...
TABLES = [
    ...
    (
    `id` INTEGER PRIMARY KEY NOT NULL,
    `buildsetid` INTEGER NOT NULL, -- every BuildRequest has a BuildSet
    ...
    -- use '%s-%d' % (mastername, boot_time). The next incarnation of that
    -- master can recognize the old claim and restart those builds quickly
    -- at reboot. Other masters must wait for the claim to expire.
    `complete` INTEGER default 0, -- complete=0 means 'pending'
    `results` SMALLINT, -- 0=SUCCESS,1=WARNINGS,etc, from status/builder.py
     -- only valid when complete==1
    `submitted_at` INTEGER NOT NULL,
    `complete_at` INTEGER
    );""",
#    """CREATE TABLE buildrequest_properties
#    (
...
class DBConnector(util.ComparableMixin):
    ...
    def changeEventGenerator(self, branches=[], categories=[]):
        q = "SELECT changeid FROM changes"
        args = []
        if branches or categories:
            q += " WHERE"
            if branches:
                q += " branch IN " + self.parmlist(len(branches))
                args.extend(list(branches))
            if branches and categories:
                q += " AND"
            if categories:
                q += " category IN " + self.parmlist(len(categories))
                args.extend(list(branches))
